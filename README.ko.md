...
- 포트 화면: macOS에서는 `lsof`, Linux에서는 `ss`, Windows에서는 `netstat`으로 listening TCP port를 보여주며, Summary/Detail 패널, 프로세스 메타데이터, kill/restart 액션을 제공합니다.
...
Windows에서는 탭 전환이 막히지 않도록 port detail은 Detail 패널에서만 불러오고, RX/TX adapter statistics는 더 낮은 주기로 갱신합니다.
...
Windows WinGet, 최초 manifest가 `microsoft/winget-pkgs`에 merge된 이후:

```powershell
winget install Choihunchul.Lazyifconfig
```

그 전에는 GitHub Releases의 Windows `.zip`을 사용하거나 Cargo로 source install을 사용하세요.
...
- Ports: `Tab`으로 Summary와 Detail을 전환합니다. Windows에서는 Detail 선택 시에만 process metadata를 불러옵니다.
